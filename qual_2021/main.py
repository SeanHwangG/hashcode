...
def to_output(file_name: str, intersections: List["Intersection"]):
  with open(file_name, 'w') as f:
    intersections = list(filter(lambda intersection : len(intersection.schedules) != 0, intersections))
...
def eunsoo_solution(D: int, F:int, intersections:List['Intersection'], cars: List['Car']):
  ...
  intersect_id2intersection = Intersection.id2intersection
  for i in intersections:
    i.incoming = []
  street_id2street = {}
  for street in street2interect_id:
    L = street2latency[street]
    intersect = intersect_id2intersection[street2interect_id[street]]
  ...
  for c in cars:
    prev = D
    for s in c.streets:
      if street_id2street[s].count < 30:
        street_id2street[s].score += (prev - street_id2street[s].L)/D
      street_id2street[s].count += 1
      prev -= street_id2street[s].L

  for i in intersections:
    schedule = [(s.id, s.score) for s in i.incoming]
    scores = [s[1] for s in schedule]
    min_score = min(scores)
    periods = [math.ceil((s/max(1, min_score))/max(1, sum(scores) * D)) for s in scores]
